...
class WebJudger:
    ...
    def check_web_price(response):
        cloth_df = pd.read_csv(config.cloth_path, encoding='gbk')
        results = []
        # 热门服装ID列表
        hot_cloth_ids = [
            7244,
        ...
        for name, price, price1m in zip(
            result_xianding.get("限定", {}).get("names", []),
        ...
        # 获取最大值
        max_price = (
            limited_gold_names_with_prices[0][1]
            if limited_gold_names_with_prices
            else None
        )

        # 将排序后的结果转换为字符串
        sorted_limited_gold_names_with_prices = [
            f"{name}{int(price)}" for name, price in limited_gold_names_with_prices
        ]
        ...
        for name, price, price1m in zip(
            result_xianding.get("限定真理", {}).get("names", []),
        ...
        # 将排序后的结果转换为字符串
        sorted_limited_truth_names_with_prices = [
            f"{name}{int(price)}" for name, price in limited_truth_names_with_prices
        ]
        ...
        all_limited_cloth_prices1m = round(
            (
                ...
                + matched_data[matched_data["xianding"] == "限定真理"][
                    "xdprice1m"
                ].sum()
            ),
            1,
        ...
        # 计算限定总数
        result["限定总数"] = result["限定金个数"] + result["限定真理个数"]
        ...
        for (name, price), (_, price1m) in zip(
            limited_gold_names_with_prices, limited_gold_names_with_prices1m
        ):
