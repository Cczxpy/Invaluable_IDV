...
xlsx_file = r"database\第五人格藏宝阁制作版.xlsx"
...
def making_words():
    ...
    ocr = PaddleOCR(use_angle_cls=True, lang="ch", 
                    det_model_dir=r"pretrained_models\ch_PP-OCRv4_det_infer",
                    rec_model_dir=r"pretrained_models\ch_PP-OCRv4_rec_infer",
                    rec_char_dict_path=r"need\ppocrv4_doc_dict.txt",
                    use_gpu=True,
    ...
    for idx,name_now in enumerate(txts_tmp):
        for card in cards:
            ...
            if card['name'] == name_now :
                ...
                usd[card['name']] = True
                break
            elif cmp(card['name'],name_now) == True:
                total += card['price_new']
                decc *= 0.99
                boxes.append(boxes_tmp[idx])
...
def process_image(input_image):
    ...
    # ans = total * decc
    # ans = math.floor(ans)
    description = f"图中所标注皮肤总价格为{total}建议乘折扣系数{decc}，因此我给出基础价格：{ans}"
...
with gr.Blocks() as demo:
    ...
    query_button.click(process_image, inputs=image_input, outputs=[text_output, image_output])
